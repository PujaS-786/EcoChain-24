...
── 0e. Create .env file ──
  ...
  NOTE: gemini-1.5-* models are retired (return 404). Use gemini-3.5-flash.
  For tighter free-tier quota, gemini-3.5-flash-lite has higher daily limits.

✋ Wait for confirmation that .env is saved before continuing.

Report: "✅ System ready. Python OK, uv OK, agents-cli OK, skills OK, .env set with default model gemini-3.5-flash."
...
─────────────────────────────────────────────────────────────────────────────
  ...
  - Verify/copy the `.env` file from the workspace root into `<project-name>/.env` (with GOOGLE_API_KEY, GOOGLE_GENAI_USE_VERTEXAI=False, and GEMINI_MODEL=gemini-3.5-flash)
...
them up front in every build:
  ...
  2. DEAD MODEL → 404 at first query.
     Use gemini-3.5-flash (or -lite). Never gemini-1.5-* (retired).
...
═══════════════════════════════════════════════════════════════════════════════
  ...
  @dataclass
  class AgentConfig:
      # Reads model from environment GEMINI_MODEL. Default gemini-3.5-flash (the 1.5 family is retired and returns 404). Use gemini-3.5-flash-lite for tighter free-tier quota.
